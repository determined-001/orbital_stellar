import type { RetryQueue, RetryJob } from "./types.js";
...
export class PostgresRetryQueue implements RetryQueue {
  ...
  /**
   * Enqueues a webhook delivery retry job.
   */
  async enqueue(url: string, event: any, attempt: number, nextAttemptAt: Date): Promise<void> {
    const queryText = `
      ...
      VALUES ($1, $2, $3, $4)
    `;
    await this.pg.query(queryText, [url, JSON.stringify(event), attempt, nextAttemptAt]);
  }

  /**
   * Dequeues up to `limit` due jobs, locking them atomically with SKIP LOCKED.
   */
  async dequeue(limit: number, leaseDurationMs: number): Promise<RetryJob[]> {
    const lockedUntil = new Date(Date.now() + leaseDurationMs);
    
    // We use a CTE to atomically select and lock the rows, then update their lease.
    // This is 100% thread-safe across multiple concurrent workers.
    const queryText = `
      WITH next_jobs AS (
        SELECT id
        FROM ${this.tableName}
