...
import { TriggerEvaluator, WorkerState } from "../src/TriggerEvaluator.js";
import { Schedule, nextDue, dueTimesBetween } from "../src/schedule.js";
...
describe("schedule", () => {
  ...
  test("nextDue cron", () => {
    const schedule: Schedule = { type: "cron", expression: "0 0 * * *", timezone: "UTC" };
    const after = new Date("2024-01-01T00:00:00Z");
  ...
  test("cron nextDue respects timezone across DST boundary", () => {
    ...
    expect(nextDue(schedule, before).toISOString()).toBe("2024-03-09T07:00:00.000Z");
...
describe("TriggerEvaluator", () => {
  ...
  test("poll respects minPollIntervalMs", async () => {
    ...
    const clock = { now: () => now };
    const evaluator = new TriggerEvaluator({ ledgerSource, clock, minPollIntervalMs: 1000 });
